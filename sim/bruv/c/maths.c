...
i32 uptopow2_i32(i32 x) { return (i32)uptopow2_u32(x); }
i64 uptopow2_i64(i64 x) { return (i64)uptopow2_u64(x); }
...
void fit_quadratic_f64(f64* rstr a, f64* rstr b, f64* rstr c, f64 x0, f64 y0,
...
void fit_cubic_f64(f64* rstr a, f64* rstr b, f64* rstr c, f64* rstr d, f64 x0,
    ...
    f64 x2_3 = x2_2*x2;
    f64 x3_3 = x3_2*x3;
    f64 den = x0_3*(x1_2*(x2-x3)-x2_2*(x1-x3)+x3_2*(x1-x2))
    ...
    *d = x0_3*(x1_2*(x2*y3-x3*y2)-x2_2*(x1*y3-x3*y1)+x3_2*(x1*y2-x2*y1))
       - x1_3*(x0_2*(x2*y3-x3*y2)-x2_2*(x0*y3-x3*y0)+x3_2*(x0*y2-x2*y0))
       + x2_3*(x0_2*(x1*y3-x3*y1)-x1_2*(x0*y3-x3*y0)+x3_2*(x0*y1-x1*y0))
       - x3_3*(x0_2*(x1*y2-x2*y1)-x1_2*(x0*y2-x2*y0)+x2_2*(x0*y1-x1*y0));
    *a /= den;
    *b /= den;
    *c /= den;
    *d /= den;
...
void fit_cubic_f32(f32* rstr a, f32* rstr b, f32* rstr c, f32* rstr d, f32 x0,
    ...
    f32 x2_3 = x2_2*x2;
    f32 x3_3 = x3_2*x3;
    f32 den = x0_3*(x1_2*(x2-x3)-x2_2*(x1-x3)+x3_2*(x1-x2))
            - x1_3*(x0_2*(x2-x3)-x2_2*(x0-x3)+x3_2*(x0-x2))
            + x2_3*(x0_2*(x1-x3)-x1_2*(x0-x3)+x3_2*(x0-x1))
            - x3_3*(x0_2*(x1-x2)-x1_2*(x0-x2)+x2_2*(x0-x1));
    *a = y0*(x1_2*(x2-x3)-x2_2*(x1-x3)+x3_2*(x1-x2))
