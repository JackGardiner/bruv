...
public class OrientedContour
{
                        ...
                        out Vector3 vecNormal)
    {
        ...
        vecPt = VecOperations.vecTranslatePointOntoFrame(m_frm, VecOperations.ConvertTo3D(vecPt2d));

        // vecNormal = m_frm.vecDirToWorld(vecN2d);
        vecNormal = VecOperations.vecTranslateDirectionOntoFrame(m_frm, VecOperations.ConvertTo3D(vecPt2d));
    }
...
{
    private readonly Vector2[] _v;   // closed: last == first
    private readonly int _nSeg;

    public float fLength { get; } = 1.0f; // keep it normalized

    public PolyContour2d(in Vector2[] avecVertices)
    {
        if (avecVertices is null) throw new ArgumentNullException(nameof(avecVertices));
        if (avecVertices.Length < 3) throw new ArgumentException("Need ≥ 3 vertices.", nameof(avecVertices));

        bool closed = avecVertices[0] == avecVertices[^1];
        if (closed)
            _v = (Vector2[])avecVertices.Clone();
        else
        {
            _v = new Vector2[avecVertices.Length + 1];
            Array.Copy(avecVertices, _v, avecVertices.Length);
            _v[^1] = _v[0];
        }
        _nSeg = _v.Length - 1;
    }

    public void PtAtT(in float t, out Vector2 pt, out Vector2 n)
    {
        // Map t = [0,1) uniformly across segments
        float tt = t / MathF.Max(fLength, 1e-12f);  // normalize if fLength != 1 in the future
        tt -= MathF.Floor(tt);                      // wrap
        float s = tt * _nSeg;                       // segment-space
        int i = (int)MathF.Min(MathF.Floor(s), _nSeg - 1);
        float u = s - i;                            // local [0,1)

        Vector2 a = _v[i];
        Vector2 b = _v[i + 1];

        // Point (linear on the edge)
        pt = a + u * (b - a);

        // Piecewise-constant outward normal from edge (rotate edge 90° CCW, then normalize)
        Vector2 e = b - a;
        if (e.LengthSquared() > 0f)
        {
            Vector2 nn = new(e.Y, -e.X); // CCW perp for outward (assuming CCW vertex order)
            float len = nn.Length();
            n = len > 0f ? nn / len : Vector2.UnitY;
        }
        else
        {
            n = Vector2.UnitY; // degenerate edge fallback
        }
    }
}
...
{
    // Axis-aligned, centered at origin
    public Rectangle(float fLengthA, float fLengthB)
        : base(
        [
            new Vector2(-0.5f * fLengthA, -0.5f * fLengthB),
            new Vector2( 0.5f * fLengthA, -0.5f * fLengthB),
            new Vector2( 0.5f * fLengthA,  0.5f * fLengthB),
            new Vector2(-0.5f * fLengthA,  0.5f * fLengthB),
        ])
    {
        if (fLengthA <= 0f || fLengthB <= 0f)
